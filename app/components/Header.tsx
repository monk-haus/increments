...
export default function Header() {
    ...
    const textColor = desktopWorkOpen
        ? "text-[#1A1A1A]"
        : isDarkPage
            ? "text-[#F5F5F0]"
            : "text-[#1A1A1A]";
    ...
    return (
        <header className="hidden md:block fixed top-0 left-0 right-0 z-50 px-10 py-8 md:px-20 md:py-10 box-border bg-transparent">
            ...
            <div className="w-full max-w-[1920px] mx-auto hidden md:grid grid-cols-5 items-start">
                <div className="text-left z-50">
                    <Link href="/" className="group inline-block" onClick={() => setDesktopWorkOpen(false)}>
                        <motion.span
                            className={`text-[9px] font-bold uppercase tracking-widest hover:text-[#8B3A2F] transition-colors block ${textColor}`}
                    ...
                    <div key={link.name} className="text-center z-50">
                        {link.isDropdown ? (
                            <motion.button
                                ...
                                animate={preloaderComplete ? "visible" : "hidden"}
                            >
                                <span className={`text-[9px] font-bold uppercase tracking-widest block transition-transform duration-300 group-hover:-translate-y-full ${desktopWorkOpen ? 'text-[#8B3A2F]' : textColor}`}>
                                    {link.name}
                                </span>
                                <span className="absolute top-0 left-0 w-full text-[9px] font-bold uppercase tracking-widest text-[#8B3A2F] block transition-transform duration-300 translate-y-full group-hover:translate-y-0">
                                    {link.name}
                                </span>
                            </motion.button>
                        ) : (
                            ...
                            >
                                <Link href={link.href} className="group inline-block relative overflow-hidden">
                                    <span className={`text-[9px] font-bold uppercase tracking-widest block transition-transform duration-300 group-hover:-translate-y-full ${textColor}`}>
                                        {link.name}
                                    </span>
                                    <span className="absolute top-0 left-0 w-full text-[9px] font-bold uppercase tracking-widest text-[#8B3A2F] block transition-transform duration-300 translate-y-full group-hover:translate-y-0">
                                        {link.name}
                            ...
                            <div className="col-span-1 flex flex-col items-start pl-[calc(50%-26px)]">
                                <div className="flex flex-col gap-1 text-left max-h-[70vh] overflow-y-auto no-scrollbar w-full">
                                    {recentWork.map((project, index) => (
                                        ...
                                        >
                                            <span className="text-[9px] font-bold uppercase tracking-widest group-hover:text-[#8B3A2F] text-[#1A1A1A] transition-colors block py-0.5">
                                                {project.name}
                                            </span>
                                        </Link>
                                    ))}
